...
describe("compareVersions", () => {
  ...
  it("normalizes voco-prefixed release tags before comparing", () => {
    expect(compareVersions("voco.2026.0.5", "2026.0.4")).toBeGreaterThan(0);
    expect(compareVersions("voco.2026.0.5", "2026.0.5")).toBe(0);
  });
});
  ...
  const releases = [
    {
      draft: false,
      prerelease: true,
      html_url: "https://github.com/sergiopesch/voco/releases/tag/voco.2026.0.6-beta.1",
      name: "VOCO 2026.0.6 beta 1",
      published_at: "2026-04-02T09:00:00Z",
      tag_name: "voco.2026.0.6-beta.1",
    },
    {
      draft: false,
      prerelease: false,
      html_url: "https://github.com/sergiopesch/voco/releases/tag/voco.2026.0.5",
      name: "VOCO 2026.0.5",
      published_at: "2026-03-28T09:00:00Z",
      tag_name: "voco.2026.0.5",
    },
  ];

  it("prefers the newest stable release on the stable channel", () => {
    expect(selectReleaseForChannel(releases, "stable")?.version).toBe("2026.0.5");
  });

  it("allows prerelease builds on the beta channel", () => {
    expect(selectReleaseForChannel(releases, "beta")?.version).toBe("2026.0.6-beta.1");
  });
});
...
describe("update cache helpers", () => {
  it("returns a recent cached state for the matching channel and version", async () => {
    const cacheState = {
      ...
      currentVersion: "2026.0.5",
      latestRelease: {
        version: "2026.0.5",
        name: "VOCO 2026.0.5",
        url: "https://github.com/sergiopesch/voco/releases/tag/voco.2026.0.5",
    ...
    await expect(readCachedUpdateState("stable", "2026.0.5")).resolves.toMatchObject({
      status: "up-to-date",
    });
  });
});
