...
impl PlannerState {
    ...
    /// Restore numerically possible successful progress, not session identity or
    /// proof that prior decoding/delivery actually happened. Callers must bind
    /// this state to the request's session and retained source-audio coordinates.
    pub fn from_progress(
        next_input_start: usize,
        previous_decoded_end: usize,
        successful_sequence: u64,
        finalized: bool,
    ) -> Result<Self, PlannerError> {
        if successful_sequence == 0 {
            return if next_input_start == 0 && previous_decoded_end == 0 && !finalized {
                Ok(Self::new())
            } else {
                Err(PlannerError("invalid initial progress"))
            };
        }
        let overlap = previous_decoded_end
            .checked_sub(next_input_start)
            .ok_or(PlannerError("progress contains a gap"))?;
        let count = u128::from(successful_sequence);
        let cursor = next_input_start as u128;
        let possible = if finalized {
            // Final receipts are strictly shorter than HORIZON. All preceding
            // receipts are nonfinal; a legacy predecessor also requires >OVERLAP
            // final samples, which lies inside this union of reachable intervals.
            // Ordered all-cut prefixes and cut-then-legacy prefixes still cover
            // this complete final-position interval; see REACHABILITY.md.
            let prefix_count = count - 1;
            overlap == 0
                && cursor > prefix_count * MIN_SIDE as u128
                && cursor <= prefix_count * (HORIZON - OVERLAP) as u128 + (HORIZON - 1) as u128
        } else if overlap == OVERLAP {
            // Last successful receipt must have been the exact legacy stride.
            next_input_start
                .checked_sub(HORIZON - OVERLAP)
                .is_some_and(|prefix| possible_nonfinal_stride_sum(successful_sequence - 1, prefix))
        } else if overlap == 0 {
            // A nonfinal disjoint state can only follow an all-cut prefix:
            // any legacy stride latches overlap until Final. Each cut is on
            // the FRAME/2 grid within MIN_SIDE..HORIZON-MIN_SIDE.
            next_input_start.is_multiple_of(FRAME / 2)
                && cursor >= count * MIN_SIDE as u128
                && cursor <= count * (HORIZON - MIN_SIDE) as u128
        } else {
            false
        };
        if !possible {
            return Err(PlannerError("impossible successful progress"));
        }
        Ok(Self {
            next_start: next_input_start,
            previous_end: previous_decoded_end,
            sequence: successful_sequence,
            finalized,
        })
    }
    /// Number of committed successful receipts, not a transport attempt ID.
    pub fn successful_sequence(&self) -> u64 {
        self.sequence
    }
    /// True only after committing a short Final receipt. An exact-horizon
    /// legacy ending can instead be complete with an overlap-only suffix.
...
// Checked in u128 so even a malicious u64 sequence cannot overflow intermediate
// arithmetic on supported usize widths. This is geometry, not acoustic validation.
fn possible_nonfinal_stride_sum(count: u64, samples: usize) -> bool {
    if count == 0 {
        return samples == 0;
    }
    let count = u128::from(count);
    let samples = samples as u128;
    let maximum = count * (HORIZON - OVERLAP) as u128;
    samples.is_multiple_of((FRAME / 2) as u128)
        && samples >= count * MIN_SIDE as u128
        && (samples == maximum || samples <= maximum - (MIN_SIDE - OVERLAP) as u128)
}
...
mod tests {
    ...
    #[test]
    fn restored_progress_replans_identically_through_mixed_seams() {
        let mut audio = varying(3 * HORIZON + 37);
        audio[800_000..806_400].fill(0.0);
        let mut state = PlannerState::new();
        loop {
            let restored = PlannerState::from_progress(
                state.next_input_start(),
                state.previous_decoded_end(),
                state.successful_sequence(),
                state.is_finalized(),
            )
            .unwrap();
            ...
            let suffix = &audio[state.next_input_start()..];
            let expected = plan_next(&state, suffix, true).unwrap();
            assert_eq!(plan_next(&restored, suffix, true).unwrap(), expected);
            let Some(receipt) = expected else {
                break;
            };
            state.commit(&receipt).unwrap();
        }
        assert_eq!(state.previous_decoded_end(), audio.len());
    }
    #[test]
    fn exact_horizon_terminal_overlap_suffix_is_not_an_extra_success() {
        let mut state = PlannerState::new();
        let audio = varying(HORIZON);
        let receipt = plan_next(&state, &audio, true).unwrap().unwrap();
        state.commit(&receipt).unwrap();
        assert_eq!(state.successful_sequence(), 1);
        assert!(!state.is_finalized());
        let restored = PlannerState::from_progress(HORIZON - OVERLAP, HORIZON, 1, false).unwrap();
        assert_eq!(state, restored);
        assert_eq!(
            plan_next(&restored, &audio[HORIZON - OVERLAP..], true).unwrap(),
            None
        );
        assert!(PlannerState::from_progress(HORIZON - OVERLAP, HORIZON, 1, true).is_err());
        assert!(PlannerState::from_progress(HORIZON, HORIZON, 1, true).is_err());
    }
    #[test]
    fn short_final_and_overlap_plus_one_sample_restore() {
        for n in [1, OVERLAP - 1, HORIZON - 1, HORIZON + 1] {
            let audio = varying(n);
            let receipts = full(&audio);
            let mut state = PlannerState::new();
            for r in &receipts {
                state.commit(r).unwrap();
            }
            assert!(state.is_finalized());
            assert_eq!(
                PlannerState::from_progress(n, n, receipts.len() as u64, true).unwrap(),
                state
            );
        }
    }
    #[test]
    fn initial_gaps_wrong_overlaps_and_impossible_sequences_fail() {
        assert_eq!(
            PlannerState::from_progress(0, 0, 0, false).unwrap(),
            PlannerState::new()
        );
        for values in [
            (0, 0, 0, true),
            ...
            (0, 0, 1, false),
            (0, 0, 1, true),
            (HORIZON, HORIZON - 1, 1, false),
            (HORIZON - OVERLAP, HORIZON - 1, 1, false),
            (HORIZON - OVERLAP, HORIZON + 1, 1, false),
            (MIN_SIDE + 1, MIN_SIDE + 1, 1, false),
            (HORIZON - OVERLAP, HORIZON - OVERLAP, 1, false),
            (MIN_SIDE, MIN_SIDE, 2, false),
            (1, 1, u64::MAX, true),
            (usize::MAX, usize::MAX, u64::MAX, false),
        ] {
            assert!(
                PlannerState::from_progress(values.0, values.1, values.2, values.3).is_err(),
                "accepted {values:?}"
            );
        }
    }
    #[test]
    fn reachable_stride_interval_edges_and_forbidden_maximum_gap() {
        for n in [MIN_SIDE, HORIZON - MIN_SIDE] {
            assert!(PlannerState::from_progress(n, n, 1, false).is_ok());
        }
        for count in [1, 2, 3, 100] {
            let max = count * (HORIZON - OVERLAP);
            assert!(possible_nonfinal_stride_sum(count as u64, max));
            assert!(possible_nonfinal_stride_sum(
                count as u64,
                max - (MIN_SIDE - OVERLAP)
            ));
            assert!(!possible_nonfinal_stride_sum(count as u64, max - FRAME / 2));
            assert!(!possible_nonfinal_stride_sum(
                count as u64,
                count * MIN_SIDE - FRAME / 2
            ));
            assert!(PlannerState::from_progress(max, max + OVERLAP, count as u64, false).is_ok());
        }
    }
    ...
    #[test]
    fn restored_disjoint_progress_requires_an_all_cut_prefix() {
        for count in [1_usize, 2, 3, 100] {
            let maximum = count * (HORIZON - MIN_SIDE);
            assert!(PlannerState::from_progress(maximum, maximum, count as u64, false).is_ok());
            assert!(PlannerState::from_progress(
                maximum + FRAME / 2,
                maximum + FRAME / 2,
                count as u64,
                false
            ...
            .is_err());
            let minimum = count * MIN_SIDE;
            assert!(PlannerState::from_progress(minimum, minimum, count as u64, false).is_ok());
        }
        // One legacy stride followed by a cut used to admit this disjoint sum.
        assert!(PlannerState::from_progress(920_000, 920_000, 2, false).is_err());
        assert!(PlannerState::from_progress(920_000, 936_000, 2, false).is_ok());
    }
    #[test]
    fn restored_final_union_keeps_both_endpoint_bounds() {
        for count in [1_usize, 2, 3, 100] {
            let low = (count - 1) * MIN_SIDE + 1;
            let high = (count - 1) * (HORIZON - OVERLAP) + HORIZON - 1;
            assert!(PlannerState::from_progress(low, low, count as u64, true).is_ok());
            assert!(PlannerState::from_progress(high, high, count as u64, true).is_ok());
            assert!(PlannerState::from_progress(low - 1, low - 1, count as u64, true).is_err());
            assert!(PlannerState::from_progress(high + 1, high + 1, count as u64, true).is_err());
        }
    }
}
