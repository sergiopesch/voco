...
impl TrayIcon {
    pub fn set_icon_path(&mut self, path: &Path) -> crate::Result<()> {
        if !path.is_absolute() || !path.is_file() {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Expected an absolute icon file").into());
        }
        self.indicator.set_icon_full(&path.to_string_lossy(), "tray icon");
        Ok(())
    }
