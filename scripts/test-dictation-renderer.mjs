...
export const getDesktopPasteStatus = async () => ({enabled:Boolean(window.desktopPaste),streamingEnabled:Boolean(window.desktopStream),targetToken:window.desktopTargetToken??null,available:!window.pasteUnavailable,detail:'Paste helper unavailable'});
...
export const askOpenClawAgent = async () => ({response:'Assistant answer'});
export const askLocalLlmAgent = async () => { calls.push(['askLocalLlmAgent']); if(window.deferAgent) await new Promise(resolve=>window.resolveAgent=resolve); window.agentReturned=true; return {response:'Assistant answer'}; };
export const enhanceTranscript = async (text) => {if(window.deferEnhancement) await new Promise(resolve=>window.resolveEnhancement=resolve);return {text:window.enhancedText ?? text,usedEnhancement:window.enhancedText !== undefined,warning:null};};
export const saveDebugDictationCapture = async () => null;
export const speakOpenClawResponse = async () => {};
export const testLocalLlm = async () => ({ok:true});
...
const {DictationStatusOverlay} = await import('/src/components/DictationStatusOverlay.tsx');
...
const config = {hotkey:'Alt+D',selectedMic:null,insertionStrategy:'auto',transcriptTarget:'cursor',liveCursorMode:'final-text-only',openclawAgent:'main',openclawPromptPrefix:'',transcriptEnhancement:'off',localLlmEndpoint:'http://127.0.0.1:8080/v1/chat/completions',localLlmModel:null,onboardingCompleted:true,updateChannel:'stable',installChannel:'github-release',voiceProfile:'default'};
...
function Harness() {
 ...
 if(window.harnessOverlay) return React.createElement(DictationStatusOverlay,{status:state.status,interimTranscript:state.interimTranscript,transcript:state.transcript,audioLevel:state.audioLevel,cursorDeliveryState:hook.cursorDeliveryState,captureNotice:state.captureNotice,canCancel:hook.canCancel,cancellationPending:hook.cancellationPending,onCancel:()=>void hook.cancelRecording()});
 return React.createElement(React.Fragment,null,

 React.createElement(ControlPanel,{surface:'popover',onboardingStep:0,config:state.config,errorMessage:state.error,statusLabel:deriveStatusLabel({configurationError:false,hasRecovery:Boolean(state.recovery),manualTranscriptReady:state.recovery?.kind==='manual-copy',cursorDeliveryState:hook.cursorDeliveryState,cursorRequired:true,cursorSetupState:'ready',dictationStatus:state.status,isRealtimeActive:false,microphonePermission:'granted',microphoneReady:true,realtimeMuted:false,realtimeStatus:'idle'}),updateState:state.updateState,runtimeDiagnostics:null,dictationStatus:state.status,cursorDeliveryState:hook.cursorDeliveryState,transcript:state.transcript,rawTranscript:state.rawTranscript,recovery:state.recovery,captureNotice:state.captureNotice,canCancelDictation:hook.canCancel,cancellationPending:hook.cancellationPending,onCancelDictation:()=>void hook.cancelRecording(),onRetryRecovery:()=>void hook.retryRecovery(),onDiscardRecovery:hook.discardRecovery,requestedSection:'General',requestedSectionRequestId:0,isRealtimeActive:false,isRealtimeMuted:false,realtimeActivationAllowed:false,realtimeStatus:'idle',realtimeDetail:'',realtimeError:null,realtimeLevel:0,selectedDeviceId:null,availableDevices:[],microphonePermission:'granted',onSurfaceChange:()=>{},onOnboardingStepChange:()=>{},onConfigChange:noop,onRefreshDevices:noop,onRequestMicrophoneAccess:noop,onCheckForUpdates:noop,onOpenReleasePage:noop,onRefreshRuntimeDiagnostics:noop,onOpenSettings:noop,onToggleRealtime:()=>{}}));
}
...
await load(); await page.evaluate(()=>{window.enhancedText='Polished result.';window.store.getState().setConfig({...window.store.getState().config,transcriptEnhancement:'conservative'});});
await start(); await stop(); await recovered();
await page.getByRole('button',{name:'Copy original recognition',exact:true}).click();
assert.equal(await page.evaluate(()=>window.copiedText),'Recovered words for manual review.');
assert.equal(await page.evaluate(()=>window.store.getState().transcript),'Polished result.');
results.push('Successful enhancement retains original recognition independently and its recovery Copy action works.');
await load(); await page.evaluate(()=>{window.enhancedText='';window.store.getState().setConfig({...window.store.getState().config,transcriptTarget:'local-agent',transcriptEnhancement:'commands-only'});});
await start(); await stop(); await page.waitForFunction(()=>window.store.getState().status==='idle');
assert.equal(await page.evaluate(()=>window.store.getState().transcript),'');
assert(!(await page.evaluate(()=>window.nativeCalls)).some(c=>['askLocalLlmAgent','commitOwnedPreedit','insertText'].includes(c[0])));
results.push('An intentional empty command result does not restore discarded text or invoke an assistant.');
...
await load(); await page.evaluate(()=>{window.lease=true;window.deferAgent=true;window.store.getState().setConfig({...window.store.getState().config,transcriptTarget:'local-agent'});});
await start(); await stop(); await page.waitForFunction(()=>Boolean(window.resolveAgent));
await page.evaluate(()=>{window.reactRoot.unmount();window.store.getState().setTranscript('Replacement view');window.store.getState().setStatus('idle');window.resolveAgent();});
await page.waitForFunction(()=>window.agentReturned);
await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
assert.equal(await page.evaluate(()=>window.store.getState().transcript),'Replacement view');
assert.equal(await page.evaluate(()=>window.store.getState().status),'idle');
assert(!(await page.evaluate(()=>window.nativeCalls)).some(c=>c[0]==='commitOwnedPreedit'));
results.push('An assistant response arriving after unmount cannot overwrite replacement state or insert text.');
...
await load(); await page.setViewportSize({width:460,height:196});
await page.evaluate(()=>{window.harnessOverlay=true;window.deferInference=true;});
await start(); await stop(); await page.waitForFunction(()=>Boolean(window.resolveInference));
await page.getByRole('button',{name:'Cancel',exact:true}).click();
await page.getByRole('button',{name:'Cancelling…',exact:true}).waitFor();
await screenshot('04-processing-cancel-overlay.png');
const cancelBounds = await page.getByRole('button',{name:'Cancelling…',exact:true}).boundingBox();
assert(cancelBounds && cancelBounds.x>=0 && cancelBounds.y>=0 && cancelBounds.x+cancelBounds.width<=460 && cancelBounds.y+cancelBounds.height<=196);
await page.evaluate(()=>window.resolveInference()); await recovered();
results.push('The real compact processing overlay renders Cancel accessibly within its 460 by 196 surface and responds without native focus control.');
...
await load(); await page.evaluate(()=>{window.lease=true;window.deferEnhancement=true;window.enhancedText='Changed wording';window.store.getState().setConfig({...window.store.getState().config,transcriptEnhancement:'conservative'});});
await start(); await stop(); await page.waitForFunction(()=>Boolean(window.resolveEnhancement));
await page.getByRole('button',{name:'Cancel dictation',exact:true}).click();
await page.evaluate(()=>window.resolveEnhancement());await recovered();
assert.equal(await page.evaluate(()=>window.store.getState().rawTranscript),'Recovered words for manual review.');
assert.equal(await page.evaluate(()=>window.store.getState().transcript),'Recovered words for manual review.');
assert(!(await page.evaluate(()=>window.nativeCalls)).some(c=>['insertText','commitOwnedPreedit'].includes(c[0])));
results.push('Cancellation during enhancement preserves original recognition and suppresses both changed wording and cursor output.');
await load(); await page.evaluate(()=>{window.lease=true;window.deferAgent=true;window.store.getState().setConfig({...window.store.getState().config,transcriptTarget:'local-agent'});});
await start();await stop();await page.waitForFunction(()=>Boolean(window.resolveAgent));
await page.getByRole('button',{name:'Cancel dictation',exact:true}).click();
await page.evaluate(()=>window.resolveAgent());await recovered();
assert.equal(await page.evaluate(()=>window.store.getState().transcript),'Recovered words for manual review.');
assert(!(await page.evaluate(()=>window.nativeCalls)).some(c=>['insertText','commitOwnedPreedit'].includes(c[0])));
results.push('Cancellation during an assistant request keeps recognized words and suppresses the late answer and cursor output.');
...
await load();
await page.waitForFunction(()=>Object.keys(window.shortcutListeners||{}).length===2);
