#!/usr/bin/env python3
"""Freeze iteration-five qualification from eight speakers never used in prior plans; no inference."""
...
parser.add_argument('--prior-qualification-plan', type=Path, required=True)
...
prior = json.loads(args.prior_qualification_plan.read_text())
assert prior.get('qualificationOnly') is True and len(prior.get('selectedSources', [])) == 8, 'Expected frozen prior eight-speaker qualification'
excluded = {e['speakerId'] for m in [base, extra] for e in m['fixtures']}
excluded.update(e['speakerId'] for e in prior['selectedSources'])
assert len(excluded) == 20, 'Expected20 previously used distinct public speakers'
...
def add(name,family,samples,source,noise=None,placement=0):
    ...
    cases.append(dict(id=name,family=family,file=path.name,sha256=sha(path),
                      ...
                      integrityRequirements=dict(maxDeletions=0,maxInsertions=0,
                        boundary=dict(prefix=source['reference'].split()[0],suffix=source['reference'].split()[-1]))))
...
for index,(source,samples) in enumerate(selected):
    add('qualification5-natural-'+source['id'],'qualification-natural',samples,source)
    output,noise = noisy(samples,len(samples),0,SEED+index)
    add('qualification5-noisy-'+source['id'],'qualification-noisy',output,source,noise)
used = set()
for index,position in enumerate([0,5,10,'tail']):
    ...
    add(f'qualification5-pause-{position}-'+source['id'],'qualification-pauses',output,source,noise,start)
assert len(cases) == 20
write(out/'combined.wav',combined)
plan = dict(schemaVersion=1,createdBeforeInference=True,qualificationOnly=True,
            selection='Next eight numeric dev-clean speaker IDs excluding eight original, four adversarial and eight prior qualification speakers; first lexicographic complete utterance <=30s per speaker.',
            priorQualificationPlanSha256=sha(args.prior_qualification_plan), priorQualificationStatus='diagnostic-only; already evaluated',
            ...
            notes=['Freeze before any candidate hypothesis; root must authorize a single qualification run after development checks.',
                   'Supplemental integrity requires zero deletions and zero insertions independently, plus exact normalized first/last reference words. Existing per-case/family WER gates remain separate and unchanged.',
                   'Eight additional LibriSpeech speakers improve corpus independence, not spontaneous speech, physical microphone, accent, language or user-population coverage.',
