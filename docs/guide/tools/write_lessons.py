...
lesson(
    ...
    [
        ...
        (
            "An empty editor can contain a placeholder",
            "A .54 first-run report led to an isolated Brave regression: an empty editable HTML block exposed a line break that disappeared when its first character arrived. Counting that placeholder as user text made readback reject the successful paste. The follow-up source candidate applies the existing rich-paragraph handling to editable HTML blocks while retaining field identity, exact text checks and literal newlines in plain text controls. This candidate is not a published release or qualification of every website; the pinned viewer remains unchanged.",
        ),
        (
            "Suggestions are not a new typing destination",
            "The owner narrowed the .54 report to Brave’s address bar. Its suggestion list announced focus while the entry still owned the keyboard. The .55 candidate resolves a popup to its focused entry only when their accessibility relationships agree; a real field change still stops delivery. WebKit also marks its web-view containers focused, so bounded discovery must reach the actual input. Tests cover browsers, native editors and terminal programs, with limits recorded per application. The .43 source viewer remains unchanged.",
        ),
        (
...
lesson(
    ...
    [
        ...
        ("Progress should follow the work", "The published .52 installer connects signal bars to measured transfer rates and a brief silver sweep to phase changes. The download runs independently: finishing work cancels animation immediately. Checksums download alongside the package and must pass before installation. Fifty-six local download trials found lower overhead than the .51 installer, but do not measure whole installations over the internet. Real Ubuntu APT tests preserve both package questions and existing configuration choices. The .52 release retains the same model and dictation behavior; its package and desktop checks are separate from these download timings. The pinned code viewer is unchanged."),
        ("Keep the installation in one place", "The .54 installer could leave old progress frames, separate stage headings and raw service messages in the terminal. The follow-up source candidate uses one compact Signal + Silver canvas through setup and replaces it with the completed state. Password and package prompts remain native and visible; a required desktop sign-out stays in the final view. This is candidate behavior, not a newly published installer. Download observations and isolated prompt tests do not qualify physical dictation."),
        ("Keep dependency upgrades accountable", "The .53 maintenance release integrates the 12 dependency PRs queued at the start of its cleanup. One shortcut update compiled successfully but would have loaded a second keyboard actor, bypassing VOCO's focus protection. The release carries the patch onto the new library and checks that both consumers share it. Compatible updates now arrive in small weekly groups; major upgrades and input dependencies remain separate. Exact-package installation and desktop tests qualify each new cut. The pinned .43 code viewer remains unchanged."),
        ("Upgrade the build as one system", "The .54 release pairs Vite 8 with React plugin 6 and Node 24 LTS. The app and its renderer tests must use the same Vite version; otherwise a passing test can exercise an older tool. Explicit output targets keep the previous compiler targets, but only the packaged WebKit app can establish how the production build behaves. Tauri also requires an updated tray library, carrying the same immutable icon-path patch. Source tests, fresh installation checks and the private GNOME journey have separate evidence. The signed release assets and public downloads are verified; the pinned .43 code viewer is unchanged."),
