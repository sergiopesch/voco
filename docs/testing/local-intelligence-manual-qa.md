...
| LI-04 | Commands only | Set Transcript enhancement to `Voice formatting commands`, dictate `first line command new paragraph command bullet point check gpio seventeen command new bullet stop` | Inserted text preserves paragraph and bullet structure |
...
| LI-06 | Conservative polish fallback | Stop `llama-server`, keep `Conservative local polish`, dictate a short sentence including a spoken command | VOCO preserves the original raw recognition before commands and shows a non-blocking local enhancement warning |
...
| LI-11 | Literal command phrases | Dictate `Please write the phrase scratch that in the title`, then `literal new paragraph` | The mention is preserved, and the literal escape produces the words `new paragraph` |
| LI-12 | Deliberate empty output | Dictate `old text command scratch that` in commands-only mode | No discarded text is reinserted; raw recognition remains recoverable |
| LI-13 | Incomplete local response | Use a synthetic local test response with content and `finish_reason: length`, then one without a finish reason | Both responses are rejected; the complete raw recognition is preserved |
...
- pass/fail for LI-01 through LI-13
...
- LI-11 through LI-13 confirm literal-command handling and complete-output fallback
