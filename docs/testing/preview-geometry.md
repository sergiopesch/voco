...
`livePreviewWindow.geometry.test.ts` checks geometry and provisional preservation. `useDictation.previewGeometry.test.ts` imports the actual hook through `?raw`, following the existing AST harness, and executes collection, deferred resampling, deferred inference, and sealing. It checks nonzero source anchors, 16/44.1/48 kHz rates, late capture preservation, and normal valid sealing. These tests mock native inference and owned-preedit delivery; they do not qualify native timestamp accuracy or real desktop insertion.
