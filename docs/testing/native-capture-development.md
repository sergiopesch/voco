...
Current builds select native capture for Wayland sessions and retain WebKit capture on X11. See the [release status](../release-candidate.md) and [22 September review](release-readiness-2026-09-22.md) for the current candidate, exact artifact evidence and open gates. The historical .43 records below do not qualify later binaries. Backend selection does not change the pinned speech model, recognition policy, mono preparation, resampling, or automatic text-delivery safeguards. Passing protocol or renderer mocks alone does not qualify audio continuity or physical microphones.
...
- `python3 scripts/test-native-capture-pulse-latency.py --output <new-directory>`: real C capture on a private Linux Pulse server, with no physical devices. Requires a C compiler, pkg-config, libpulse development files, PulseAudio utilities/server and Bubblewrap. It checks source buffering, complete original-reference waveform and bounded Stop separately; optional `--source <C-file>` permits a preserved negative control. Rust CI runs this regression and retains its evidence.

Installed Linux qualification must use a prospectively identified package, guest, source and public fixture, retain raw/prepared audio and complete coverage, and apply the existing waveform thresholds unchanged. Physical microphone qualification needs separate consent and complete reference evidence. Current publication gates are recorded in the linked release review; historical qualification is not approval for a later candidate.

## Historical .43 installed qualification

Original evidence paths in this historical section refer to external workspace
artifacts, not files included in this checkout. They are retained as provenance
identifiers rather than current download links.
